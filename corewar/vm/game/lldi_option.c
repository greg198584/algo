...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/03/04 09:33:57 by glafitte          #+#    #+#             */
/*   Updated: 2015/03/21 10:01:22 by glafitte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_ri_lldi(t_list *chmp, unsigned char *mem, int pos, int param)
{
	int	a;
	int	b;
	int	addr;
	...
	if (FIRST(mem[pos]) == 3 && TWO(mem[pos]) == 1)
	{
		...
		a = mem[(chmp->pc + addr) % MEM_SIZE];
		b = chmp->reg[mem[pos + IND + 1] - 1];
		addr = a + b;
		value = mem[(chmp->pc + addr) % MEM_SIZE];
